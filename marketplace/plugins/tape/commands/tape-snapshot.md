...
2. **Call `tape.snapshot`** with `out: "<resolved path>"`. You may also pass `task` if you can summarize what this session was about in one line; otherwise omit it and the tool will derive the task from the first user prompt.
